@section('title', 'Update Role | Employee Management')
<div>
    <h1 class="text-4xl mb-3 font-bold">Update Role</h1>
    @if (session('success'))
        <p class="bg-green-600 text-white px-2 py-5">{{ session('success') }}</p>
    @endif
    <form wire:submit.prevent='updateData({{ $role[0]->id }})' method="post">
